...
— Moto Club Online 🚗💨`;
    ...
    const handlePrintLabel = async (courier: Courier, e: React.MouseEvent) => {
        e.stopPropagation();
        setPrintCourier(courier);
                    ...
                    if (isMobile && navigator.share && navigator.canShare({ files: [file] })) {
                        try {
                            await navigator.share({
                                ...
                                text: `Shipping Label for ${courier.customerName}`
                            });
                        } catch (err) {
    ...
    const showLabelButton = (status: string | undefined) => {
        const s = status || 'Pending';
        return ['Paid', 'Packed', 'Sent', 'Shipped'].includes(s);
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
                {printCourier && <ShippingLabel ref={labelRef} courier={printCourier} />}
            </div>
                                        ...
                                        <td className="px-4 py-3 text-center flex items-center justify-center gap-2">
                                            {user?.role === 'PARTNER' ? (
                                                ...
                                                <>
                                                    ...
                                                    {showLabelButton(courier.status) && (
                                                        <button onClick={(e) => handlePrintLabel(courier, e)}
                                                            className="text-white bg-gray-700 hover:bg-gray-800 rounded-full p-2 shadow-md hover:shadow-lg transition-all" title="Share Label">
                                                            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9V2h12v7"></path><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><path d="M6 14h12v8H6z"></path></svg>
                                                        </button>
                                                    )}
                                                </>
                                            )}
                        ...
                        <div className="p-4">
                            ...
                            <div className="flex justify-between items-end">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {showLabelButton(courier.status) && (
                                        <button onClick={(e) => handlePrintLabel(courier, e)}
                                            className="bg-gray-700 text-white p-2 rounded-full shadow hover:bg-gray-800">
