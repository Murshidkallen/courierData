...
const BillingPage = () => {
    ...
    const generateInvoice = async () => {
        if (!stats) return;
        if (!confirm(`Generate invoice for ${stats.month} with amount Γé╣${stats.totalAmount}?`)) return;
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="p-4 bg-green-50 rounded-xl">
                        ...
                        <h3 className="text-3xl font-extrabold text-green-700">Γé╣{stats?.totalAmount.toFixed(2)}</h3>
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {invoices.map((inv) => (
                                        <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm font-bold text-gray-900 text-right">Γé╣{inv.amount.toFixed(2)}</td>
